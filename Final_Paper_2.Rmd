...
```{r echo=FALSE}
reg3C_df <- augment(reg3C)
ggplot(reg3C_df, aes(x = .fitted, y = .resid)) + 
...
Since the residuals are more clustered below it indicates we may have violated our assumption. In order to double check we'll perform a RESET. Since our model already contains squared values, we'll do the test both for the powers of 2 and 3

```{r}
resettest(reg3C, power = 2)
resettest(reg3C, power = 3)
```
...
u_hat <- reg3C$residuals
y_hat <- predict(reg3C)

summary(lm(u_hat^2 ~ y_hat + I(y_hat^2)  ))
```
We have an F statistic of 2.016 and a p value of 0.1373 >.05. Therefore we fail to reject the null hypothesis indicating that despite our earlier concerns, heteroskedasticity is not an issue. Due to the results of the bp and white test won't make any standard error adjustments to our model. 


```{r}
stargazer(reg3C, type = "text")
```



$$H_{0}: inc86 = 0, narr86 = 0, narr86^2 = 0, nfarr86 = 0, durat = 0$$
$$H_{1}: inc86 \neq 0, narr86 \neq 0, narr86^2 \neq, nfarr \neq, pcnv \neq 0, pcnv^2 \neq 0, durat \neq 0 $$
...
Ho_joint <- c("inc86 = 0", "narr86 = 0", "narr86sq = 0", "nfarr86 = 0", "durat = 0", "racehisp = 0", "racenon = 0")
linearHypothesis(reg3C, Ho_joint)


```

The results of the hyothesis test has a low F value of 1.1142 with ap value of 0.3563 which indicates that these values are not jointly significant so we'll remove those variables from our model, we can see the comparison with the removed variables here


```{r}
reg4 <- lm(avgsen ~  pcnv + pcnvsq, df)

stargazer(reg3C, reg4, type = "text")
```
...
## Model Interpretation and Implication REDO
