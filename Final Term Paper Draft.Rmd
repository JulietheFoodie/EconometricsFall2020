...
Now, to carry out our investigation, we will regress our dependent variable "avgsen", which is the average sentence length of a convicted criminal, on variant explanatory variables: inc86 (legal income), nfarr86 (felony arrests), pcnv (proportion of prior convictions), durat (recent unemp duration), narr86(times arrested), Black, and Hispanic. Our multiregression model would start with the following model and build on from there to test for interaction between variables and to reflect for possible increase or decrease in sentencing length in the short- and long-term. This is our original regression model:

                        Avgsen= inc86 + nfarr86 + narr86 + pcnn + durat + black + Hisanic. 
...
library(stargazer)
library(corrgram)
df <- select(df, c(avgsen, inc86, nfarr86, narr86, pcnv, durat, black, hispan))
