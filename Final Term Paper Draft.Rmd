...
### Model Bias and Robustness
                                      
We will be using Ordinary Least Square (OLS) estimating method to compute the coefficients of our variables. Our model complies with all five of OLS multiple regression assumptions. First, our regression model is linear in parameters, as illustrated by our model above. Additionally, the sampling of the observations was undertaken randomly so we've met that condition as well. The third condition 

Also, the variance of our independent variables is `r var(df$avgsen)` which is greater than zero, ***what assumption is this***
