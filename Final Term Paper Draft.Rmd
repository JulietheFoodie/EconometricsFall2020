...
$$Avgsen= \beta_{0} + \beta_10}inc86 + \beta_{2}nfarr86 + \beta_{3}narr86 + \beta_{4}pcnv + \beta_{5}durat + \vardelta_{1}race + u$$
...
```{r}
ggplot(df, aes(x = log(inc86), y = (avgsen))) + 
...
The side by side boxplots indicates that amoung the average sentence length for each number of felony arrests has essentially no association. Due to this and the ordinal nature of the data we will not be including this variable in our regression. 
...
ggplot(df, aes(x = (durat))) + 
  ...
  ggtitle("Histogram of u=Unemployment duration") +
  geom_histogram(binwidth = 5) + 
  theme_minimal()
```
...
This scatterplot illustrates a weak association between unemployment length and average sentence length. This trend indicates that longer lengths of unemployment may not necessarily lead to the commiting of harsher crimes or that a history of unemployment leads to harsher punishments when convicted. 
...
For illustrative purposes, below we include the histogram charts of our variables:
...
```{r}
hist(df$avgsen, col = "red")
hist(df$inc86, col = "blue")
hist(df$nfarr86, col = "green")
hist(df$narr86, col = "purple")
hist(df$pcnv, col = "orange")
hist(df$durat, col = "yellow")
hist(df$black, col = "gray")
hist(df$hispan, col = "dark green")
```



Our true model that we will be examining to compute our estimated coefficients using R Programming Language is the following:

                                      Avgsen= inc86 + nfarr86 + narr86 + pcnn + durat + black + Hisanic+ u          (True Model)
                                      
                                      Avgsen^= inc86^ + nfarr86^ + narr86^ + pcnn^ + durat^ + black^ + Hisanic^ (estimated model) 
                                     
                                      
We like to point out that we will be using Ordinary Least Square (OLS) estimating method to compute the coefficients of our variables. Our model complies with all of OLS multiple regression assumptions. That is, our regression model is linear in parameters, the sampling of the observations was undertaken randomly, the variance of our independent variables is greater than zero (Var(x) > 0), the expected value of our unobserved variables given our observed explanatory variables is zero (E(u|x)=0), the variance of our unobserved variables given our observed explanatory variables is constant (Var(u|x=constant)), and the population error (u) is normally distributed with zero mean and variance u ~(Normal (0,σ^2)).Having met the above assumptions, we will run several regression models to ensure that we are actually in compliance with these assumptions. 
